...
class ODESolver:
    """Solve ODE with fixed time step 'dt'.

    A linear ODE of order n is often presented in the form:
        a_n . y^n + a_(n-1) . y^(n-1) + ... + a_0 . y + c = f(t)
    This can also be written as:
        y^n = (1/a_n).[f(t) - c - a_(n-1).y^(n-1) - ... - a_0.y]
    or more generally, any ODE of order n can be written as:
        y^n = f(t, y^(n-1),...,y)
    with f an expression in the variables t, y^(n-1), ..., y
    """
    def __init__(self, f, y0, dt, t_final=None, method='RK4', tol=1.0e-6):
        """
        Initialize ODE solver.
        Params:
            - f           name of function object with signature f(t, y_list) that returns the expression of the ODE
                          in the variables t, y[n-1],..., y[0] with n the order of the ODE
...
class AdaptiveODESolver:
    ...
    def _RK5(self):
        ...
        for i in range(500):
            ...
            # error measure = root mean square of error vector 'G'
