...
class _LinSys:
    ...
    def _row_pivot(self, col_index):
        ...
        n = len(self._a)
        k = col_index
        rel_vals = np.abs(self._a[k:n, k] / self._row_scf[k:n])  # array w. relative absolute values of self._a[k:n, k]
        p = k + np.argmax(rel_vals)  # index of largest rel. value in col. k of matrix self._a
        self._check_with_tolerance(self._a[p, k])  # if too close to zero, the matrix will be singular (det a = 0)
        if p != k:  # if self._a[k, k] hasn't the largest rel. value, swap rows
            Swap.swap_rows(self._b, k, p)
            Swap.swap_rows(self._row_scf, k, p)
            Swap.swap_rows(self._a, k, p)

    def _check_with_tolerance(self, elem):
        # If during transformation (elimination or decomposition) of self._a a diagonal el. is too close to zero,
...
class GaussElimin(_LinSys):
    ...
    def _eliminate(self):
        ...
        n = len(self._a)
        for k in range(n-1):  # index k points_1 to the 'pivot row' and 'pivot column'
